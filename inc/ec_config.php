...
function wp_easycart_maybe_convert_language() {
	if ( isset( $_POST['ec_language_conversion'] ) ) {
		...
		setcookie( 'ec_translate_to', '', time( ) - 300, COOKIEPATH ? COOKIEPATH : '/', COOKIE_DOMAIN ); 
		setcookie( 'ec_translate_to', $wpeasycart_language, time( ) + ( 3600 * 24 * 30 ), COOKIEPATH ? COOKIEPATH : '/', COOKIE_DOMAIN );
...
function wp_easycart_maybe_change_currency() {
	if ( isset( $_POST['ec_currency_conversion'] ) ) {
		setcookie( "ec_convert_to", "", time( ) - 300, COOKIEPATH ? COOKIEPATH : '/', COOKIE_DOMAIN ); 
		setcookie( 'ec_convert_to', htmlspecialchars( sanitize_text_field( $_POST['ec_currency_conversion'] ), ENT_QUOTES ), time( ) + ( 3600 * 24 * 30 ), COOKIEPATH ? COOKIEPATH : '/', COOKIE_DOMAIN );
		header( 'Location: ' . sanitize_text_field( $_SERVER['REQUEST_URI'] ) );
		die;

	} else if( isset( $_GET['eccurrency'] ) ) {
		setcookie( 'ec_convert_to', '', time( ) - 300, COOKIEPATH ? COOKIEPATH : '/', COOKIE_DOMAIN ); 
		setcookie( 'ec_convert_to', htmlspecialchars( sanitize_text_field( $_GET['eccurrency'] ), ENT_QUOTES ), time( ) + ( 3600 * 24 * 30 ), COOKIEPATH ? COOKIEPATH : '/', COOKIE_DOMAIN );
